...
class HDL32E(Sensor):
    sensor_id_glob = 110
    current_frame = 0
    eliminate_frame = 1000
    def __init__(self, vehicle, world, actor_list, folder_output, transform, init_ts):
    ...
    def save(self):
        while not self.queue.empty():
            ...
            pcd.point["positions"] = o3d.core.Tensor(np.array([buffer[:]['x'], -buffer[:]['y'], buffer[:]['z']]))
            pcd_file_path = self.frame_output+"/frame_%04d.pcd" %self.i_frame
            pose_file_path = self.frame_output+"/frame_%04d.txt" %self.i_frame
            # write pcd file
            o3d.t.io.write_point_cloud(pcd_file_path, pcd, write_ascii=True)
            print("print pcd file:%s", pcd_file_path)
            # write pose file
            R_W = rotation_carla(data.transform.rotation)
            T_W = translation_carla(data.transform.location)

            with open(pose_file_path, 'a') as posfile:
            ...
            print("print txt file:%s", pose_file_path)

            self.list_trajectory.extend([struct.pack('f', T) for T in T_W])
            self.list_trajectory.append(struct.pack('f', data.timestamp))
            self.i_frame += 1
        return self.i_frame
    ...
    # - Single Return Mode: ~695,000 points per second
    def set_attributes(self, blueprint_library):
        lidar_bp = blueprint_library.find('sensor.lidar.ray_cast_semantic')
        lidar_bp.set_attribute('channels', '32')
        lidar_bp.set_attribute('range', '70.0')    # 80.0 m
        # lidar_bp.set_attribute('points_per_second', str(32/0.00004608)) # 32/0.00004608 = 694,444
        # lidar_bp.set_attribute('points_per_second', str(100000))
        lidar_bp.set_attribute('points_per_second', str(695000))
        ## rotation frequency  Rotation Rate: 5 Hz – 20 Hz
        lidar_bp.set_attribute('rotation_frequency', '10')
        # lidar_bp.set_attribute('rotation_frequency', '2')
