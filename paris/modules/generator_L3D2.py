...
class HDL32E(Sensor):
    ...
    def save(self):
        while not self.queue.empty():
            ...
            ts = data.timestamp-self.init_ts #Sensor.initial_ts
            # print("data.timestamp:%s , Sensor.initial_ts：%s"% (str(data.timestamp), str(self.init_ts)))
            ...
            self.list_ts.append(np.broadcast_to(data.timestamp, nbr_pts))
            ...
            if self.i_packet%self.packet_per_frame == 0:
                ...
                write_flag = ply.write_ply(ply_file_path, [np.float32(pts_all), np.float32(ts_all), np.uint32(semantic_all)], ['x','y','z','cos','timestamp','index','semantic'])
                if write_flag:
            ...
            with open(self.calib_output+"/full_poses_lidar.txt", 'a') as posfile:
                ...
                posfile.write(str(data.timestamp)+"\n")
